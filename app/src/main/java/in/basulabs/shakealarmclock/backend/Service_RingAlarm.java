...
public class Service_RingAlarm extends Service implements SensorEventListener, AudioFocusController.OnAudioFocusChangeListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                startForeground(notifID, buildRingNotification(), ServiceInfo.FOREGROUND_SERVICE_TYPE_SPECIAL_USE);
            } else {
        ...
        audioFocusController = new AudioFocusController.Builder(this).setAcceptsDelayedFocus(true).setAudioFocusChangeListener(this).setContentType(AudioAttributes.CONTENT_TYPE_MUSIC).setUsage(AudioAttributes.USAGE_ALARM).setPauseWhenAudioIsNoisy(false).setStream(AudioManager.STREAM_ALARM).setDurationHint(AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_EXCLUSIVE).build();

        isShakeActive = sharedPreferences.getInt(ConstantsAndStatics.SHARED_PREF_KEY_DEFAULT_SHAKE_OPERATION, ConstantsAndStatics.SNOOZE) != ConstantsAndStatics.DO_NOTHING;
        ...
        Uri chosenToneUri = alarmDetails.getParcelable(ConstantsAndStatics.BUNDLE_KEY_ALARM_TONE_URI);
        try (InputStream ignored = getContentResolver().openInputStream(Objects.requireNonNull(chosenToneUri))) {
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            NotificationChannel channel = new NotificationChannel(Integer.toString(ConstantsAndStatics.NOTIF_CHANNEL_ID_ALARM), getString(R.string.notif_channel_name_ring_alarms), importance);
    ...
    private Notification buildRingNotification() {
        ...
        Intent fullScreenIntent = new Intent(this, Activity_RingAlarm.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK).addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY).putExtras(alarmDetails);

        int flags = android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M ? PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT : PendingIntent.FLAG_UPDATE_CURRENT;
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, Integer.toString(ConstantsAndStatics.NOTIF_CHANNEL_ID_ALARM)).setContentTitle(getResources().getString(R.string.app_name)).setPriority(NotificationCompat.PRIORITY_MAX).setCategory(NotificationCompat.CATEGORY_ALARM).setSmallIcon(R.drawable.ic_notif).setContentIntent(fullScreenPendingIntent).setOnlyAlertOnce(true).setFullScreenIntent(fullScreenPendingIntent, true);

        if (alarmMessage != null) {
            builder.setContentTitle(getString(R.string.app_name)).setContentText(alarmMessage).setStyle(new NotificationCompat.BigTextStyle().bigText(alarmMessage));
        } else {
    ...
    private void ringAlarm() {
        ...
        if (!(alarmDetails.getInt(ConstantsAndStatics.BUNDLE_KEY_ALARM_TYPE) == ConstantsAndStatics.ALARM_TYPE_VIBRATE_ONLY)) {
            ...
            AudioAttributes attributes = new AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_ALARM).setContentType(AudioAttributes.CONTENT_TYPE_MUSIC).build();
    ...
    private void snoozeAlarm() {
        ...
        if (alarmDetails.getBoolean(ConstantsAndStatics.BUNDLE_KEY_IS_SNOOZE_ON)) {
            ...
            if (numberOfTimesTheAlarmHasBeenSnoozed < alarmDetails.getInt(ConstantsAndStatics.BUNDLE_KEY_SNOOZE_FREQUENCY)) {
                ...
                Intent intent = new Intent(this, Service_SnoozeAlarm.class).putExtra(ConstantsAndStatics.BUNDLE_KEY_ALARM_DETAILS, alarmDetails).putExtra(EXTRA_NO_OF_TIMES_SNOOZED, numberOfTimesTheAlarmHasBeenSnoozed);
    ...
    private void dismissAlarm() {
        ...
        if (alarmDetails.getBoolean(ConstantsAndStatics.BUNDLE_KEY_IS_REPEAT_ON, false) && repeatDays != null && repeatDays.size() > 0) {
            ...
            LocalTime alarmTime = LocalTime.of(alarmDetails.getInt(ConstantsAndStatics.BUNDLE_KEY_ALARM_HOUR), alarmDetails.getInt(ConstantsAndStatics.BUNDLE_KEY_ALARM_MINUTE));
        ...
        Intent launchIntent = getPackageManager().getLaunchIntentForPackage("net.tropicbliss.mathquiz");
        if (launchIntent != null) {
            startActivity(launchIntent);
        } else {
            Toast.makeText(this, "Unable to launch math quiz", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void stopRinging() {
        try {
            ...
            if ((alarmDetails.getInt(ConstantsAndStatics.BUNDLE_KEY_ALARM_TYPE) == ConstantsAndStatics.ALARM_TYPE_VIBRATE_ONLY) || (alarmDetails.getInt(ConstantsAndStatics.BUNDLE_KEY_ALARM_TYPE) == ConstantsAndStatics.ALARM_TYPE_SOUND_AND_VIBRATE)) {
                vibrator.cancel();
            }
            if (mediaPlayer != null) {
                mediaPlayer.stop();
    ...
    private void setAlarm(@NonNull LocalDateTime alarmDateTime) {
        ...
        Intent intent = new Intent(getApplicationContext(), AlarmBroadcastReceiver.class).setAction(ConstantsAndStatics.ACTION_DELIVER_ALARM).setFlags(Intent.FLAG_RECEIVER_FOREGROUND).putExtra(ConstantsAndStatics.BUNDLE_KEY_ALARM_DETAILS, alarmDetails);
    ...
    private void cancelPendingIntent() {
        ...
        int flags = android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M ? PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_NO_CREATE : PendingIntent.FLAG_NO_CREATE;
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (gForce >= sharedPreferences.getFloat(ConstantsAndStatics.SHARED_PREF_KEY_SHAKE_SENSITIVITY, ConstantsAndStatics.DEFAULT_SHAKE_SENSITIVITY)) {
                ...
                if (Math.abs(currTime - lastShakeTime) > MINIMUM_MILLIS_BETWEEN_SHAKES) {
                    ...
                    shakeVibration();
                    if (sharedPreferences.getInt(ConstantsAndStatics.SHARED_PREF_KEY_DEFAULT_SHAKE_OPERATION, ConstantsAndStatics.SNOOZE) == ConstantsAndStatics.SNOOZE && alarmDetails.getBoolean(ConstantsAndStatics.BUNDLE_KEY_IS_SNOOZE_ON)) {
                        snoozeAlarm();
                    } else {
                        dismissAlarm();
                    }
